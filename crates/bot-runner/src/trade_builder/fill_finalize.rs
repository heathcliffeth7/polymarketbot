...
async fn finalize_builder_fill(
    ...
    let live_gap_stop_loss_configured = order.side == "buy"
        && trade_builder_order_has_live_gap_stop_loss_metadata(repo, order.id).await?;
    if order.side == "buy"
        && (order.tp_enabled
            || order.sl_enabled
    ...
    if let Err(err) = maybe_record_confidence_ladder_fill(
        repo,
        order,
        parent_order.as_ref(),
        flow_created_payload.as_ref(),
    ...
    .await
    {
        warn!(builder_order_id = order.id, error = %err, "CONFIDENCE_LADDER_FILL_RECORD_FAILED");
    }
    if let Err(err) = maybe_record_avg_rebound_pairlock_rescue_fill(
        repo,
        order,
        flow_created_payload.as_ref(),
        actual_fill_qty.unwrap_or(canonical_entry_qty),
    ...
    .await
    {
        warn!(
            builder_order_id = order.id,
            error = %err,
            "AVG_REBOUND_PAIRLOCK_RESCUE_FILL_RECORD_FAILED"
        );
    }
    ...
    if order.side == "buy" {
        ...
        if stop_loss_surface_active && live_gap_stop_loss_configured {
            if create_trade_builder_live_gap_stop_loss_child_if_configured(
                repo,
                ws,
                order,
                canonical_entry_qty,
