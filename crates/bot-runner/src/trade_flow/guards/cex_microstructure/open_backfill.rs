...
async fn fetch_bybit_open_with_limit(
    ...
    Ok(select_window_candle_open(rows.iter(), window_start_ms, 0, 1, 1))
}
...
async fn fetch_coinbase_open(asset: &str, window_start_ms: i64) -> Result<f64> {
    ...
    let start = Utc
        ...
        .to_rfc3339();
    let end = Utc
        .timestamp_millis_opt(cex_kline_query_end_ms(window_start_ms))
        .single()
...
fn select_window_candle_open<'a>(
    ...
    rows.filter_map(|row| parse_array_candle_open(row, timestamp_index, open_index, timestamp_multiplier))
        .find(|(start_ms, _)| candle_covers(*start_ms, window_start_ms))
        .map(|(_, open)| open)
}
