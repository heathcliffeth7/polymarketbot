...
import {
  ...
  type ClaimMergeRequestBody,
  submitMergeViaBuilderRelayer,
  submitMergeViaRelayerApiKey,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as ClaimMergeRequestBody;
    ...
    const result =
      config.executionMode === 'relayer_api_key'
        ? await submitMergeViaRelayerApiKey(config, body)
        : await submitMergeViaBuilderRelayer(config, body);
    ...
    const message = err instanceof Error ? err.message : 'Failed to submit pairlock merge';
    console.error('Internal claim merge error:', err);
    return NextResponse.json(
      {
        code: 'internal_error',
        retryable: true,
