...
import {
  ...
  type ClaimFundsActivationRequestBody,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as ClaimFundsActivationRequestBody;
    ...
    const result = await submitClaimFundsActivation(config, body);
    ...
    const message = err instanceof Error ? err.message : 'Failed to activate claim funds';
    console.error('Internal claim funds activation error:', err);
    return NextResponse.json(
      {
        code: 'internal_error',
        retryable: true,
