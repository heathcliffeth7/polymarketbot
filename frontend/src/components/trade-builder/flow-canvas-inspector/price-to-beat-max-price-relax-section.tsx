...
export function PriceToBeatMaxPriceRelaxSection({
  ...
  const checked = enabled.trim().toLowerCase() !== 'false';

  return (
    <div className="space-y-2 rounded-md border border-slate-200/80 bg-slate-50/70 p-2">
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-0.5">
          <Label className="text-[10px] font-medium text-slate-600">Max Price Relax</Label>
          <p className="text-[10px] text-slate-400">Bu node icin PTB relax hesaplamasini acar/kapatir.</p>
        </div>
        <Switch
          checked={checked}
          onCheckedChange={(value) =>
            onUpdateField('priceToBeatMaxPriceRelaxEnabled', value ? 'true' : 'false')
          }
        />
      </div>
      {checked && (
      ...
      {!checked && (
        <p className="text-[10px] leading-relaxed text-slate-400 italic">
          Kapaliyken bu node PTB maxPrice relax tracking ve gevsetme uygulamaz.
        </p>
      )}
    </div>
  );
